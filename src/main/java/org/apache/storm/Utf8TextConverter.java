...
public class Utf8TextConverter implements LoadCaster {
  ...
  private DataBag consumeBag(PushbackInputStream in,
    ...
    // TODO:DataBag db = DefaultBagFactory.getInstance().newDefaultBag();
  ...
  private Tuple consumeTuple(PushbackInputStream in,
      ResourceFieldSchema fieldSchema) throws IOException {
    if (fieldSchema == null) {
      throw new IOException("Schema is null");
    }
    ...
    if (fieldSchema.getSchema() != null
      ...
      Deque<Character> level = new LinkedList<Character>(); // keep track of
                                                            // nested
                                                            // tuple/bag/map. We
                                                            // do not interpret,
                                                            // save them as
  ...
  private Map<String, Object> consumeMap(PushbackInputStream in,
    ...
    while (true) {
      ...
      Object value = null;
      if (fieldSchema != null && fieldSchema.getSchema() != null
          && mOut.size() > 0) {
        value = bytesToObject(mOut.toByteArray(), fieldSchema.getSchema()
            .getFields()[0]);
  ...
  private Object bytesToObject(byte[] b, ResourceFieldSchema fs)
      throws IOException {
    Object field;
    if (DataType.isComplex(fs.getType())) {
  ...
  public DataBag bytesToBag(byte[] b, ResourceFieldSchema schema)
    ...
    try {
      ...
      LogUtils.warn(this, "Unable to interpret value " + Arrays.toString(b)
          + " in field being "
          + "converted to type bag, caught ParseException <" + e.getMessage()
          + "> field discarded",
  ...
  public Double bytesToDouble(byte[] b) {
    ...
    try {
      ...
      LogUtils.warn(
          this,
          "Unable to interpret value " + Arrays.toString(b)
              + " in field being "
   ...
   * @param number
   *          the number to check
   * @return true if it doesn't contain any invalid characters, i.e. only
   *         contains digits and '-'
   */
  ...
  public Integer bytesToInteger(byte[] b) throws IOException {
    ...
    // See PIG-2835. Using exception handling to check if it's a double is very
    // expensive.
  ...
  public Map<String, Object> bytesToMap(byte[] b,
    ...
    try {
      ...
      LogUtils.warn(this, "Unable to interpret value " + Arrays.toString(b)
          + " in field being "
          + "converted to type map, caught ParseException <" + e.getMessage()
          + "> field discarded",
          PigWarning.FIELD_DISCARDED_TYPE_CONVERSION_FAILED, mLog);
      return null;
    }
  ...
  public Tuple bytesToTuple(byte[] b, ResourceFieldSchema fieldSchema)
    ...
    try {
      ...
      LogUtils.warn(this, "Unable to interpret value " + Arrays.toString(b)
          + " in field being "
          + "converted to type tuple, caught ParseException <" + e.getMessage()
          + "> field discarded",
          PigWarning.FIELD_DISCARDED_TYPE_CONVERSION_FAILED, mLog);
      return null;
    }
