 ...
 * Map-only splitter during multi-query optimization. The reduce physical plans
 * of the splittees become the inner plans of this operator.
 * <p>
 * Due to the recursive nature of multi-query optimization, this operator may be
 * contained in another demux operator.
 * <p>
 * The predecessor of this operator must be a POMultiQueryPackage operator which
 * passes the index (indicating which inner reduce plan to run) along with other
...
public class PODemux extends PhysicalOperator {
  ...
  /*
   * Flag indicating when a new pull should start. It's used only when receiving
   * the call from reducer's close() method in the streaming case.
   */
  ...
  @Override
  public String name() {
    return getAliasString() + "Demux [" + myPlans.size() + "] "
        + mKey.toString();
  }
  ...
  private Result runPipeline(PhysicalOperator leaf) throws ExecException {
    ...
    return (res.returnStatus == POStatus.STATUS_OK || res.returnStatus == POStatus.STATUS_ERR) ? res
        : empty;
  }
  ...
  private Result getStreamCloseResult() throws ExecException {
    ...
    while (true) {
      ...
      if (curLeaf != null
          && leaf.getOperatorKey().equals(curLeaf.getOperatorKey())) {
   ...
   * 
   * @param inCombiner
   *          true if this operator is in a combiner; false if this operator is
   *          in a reducer
   */
   ...
   * 
   * @return true if this operator is in a combiner; otherwise this operator is
   *         in a reducer
   */
