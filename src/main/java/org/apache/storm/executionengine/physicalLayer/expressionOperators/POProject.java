...
/**
 * Implements the overloaded form of the project operator. Projects the
 * specified column from the input tuple. However, if asked for tuples when the
 * input is a bag, the overloaded form is invoked and the project streams the
 * tuples through instead of the bag.
 */
public class POProject extends ExpressionOperator {
  ...
  protected Result consumeInputBag(Result input) throws ExecException {
    ...
    if (input.result instanceof DataBag) {
      ...
      if (resultSingleTupleBag) {
        ...
        // outBag = bagFactory.newDefaultBag();//NonSpillableDataBag
        outBag = new NonSpillableDataBag();//
      ...
      throw new ExecException("Cannot dereference a bag from "
          + input.result.getClass().getName(), 1129);
    }
  }
  ...
  private Tuple getRangeTuple(Tuple tuple) throws ExecException {
    ...
    if (isRangeInvalid(lastColIdx)) {
      ...
      ArrayList<Object> objList = new ArrayList<Object>(lastColIdx - startCol
          + 1);
  ...
  /**
   * Asked for Tuples. Check if the input is a bag. If so, stream the tuples in
   * the bag instead of the entire bag.
   */
  @Override
    ...
    if (!processingBagOfTuples) {
      ...
      if (columns.size() == 1) {
        try {
          ...
          // if we
          // don't catch it, we will die and the most common case for
          // the
          // exception would be because the tuple is null
          ret = null;
        }
  ...
  public ArrayList<Integer> getColumns() {
    if (isProjectToEnd) {
      throw new AssertionError(
          "Internal error. Improper use of method getColumns() in "
  ...
  public POProject clone() throws CloneNotSupportedException {
    ...
    POProject clone = new POProject(new OperatorKey(mKey.scope, NodeIdGenerator
        .getGenerator().getNextNodeId(mKey.scope)), requestedParallelism, cols);
