 ...
 * format after the combiner stage. It differs from POPackage in that it does
 * not use the index in the NullableTuple to find the bag to put a tuple in.
 * Instead, the inputs are put in a bag corresponding to their offset in the
 * tuple.
 */
public class POCombinerPackage extends POPackage {
   ...
   *          POPackage to clone.
   * @param bags
   *          for each field, indicates whether it should be a bag (true) or a
   *          simple field (false).
   */
  public POCombinerPackage(POPackage pkg, boolean[] bags) {
    super(new OperatorKey(pkg.getOperatorKey().scope, NodeIdGenerator
        .getGenerator().getNextNodeId(pkg.getOperatorKey().scope)), pkg
        .getRequestedParallelism(), pkg.getInputs());
  ...
  public void setKeyInfo(
    ...
    Pair<Boolean, Map<Integer, Integer>> lrKeyInfo = keyInfo.get(0); // assumption:
    // only
    // group
    // are
    // "combinable",
  ...
  public Result getNextTuple() throws ExecException {
    ...
    for (int i = 0; i < mBags.length; i++) {
      if (mBags[i])
        fields[i] = createDataBag(numBags);
    }
