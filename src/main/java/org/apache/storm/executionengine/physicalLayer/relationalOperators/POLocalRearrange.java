...
/**
 * The local rearrange operator is a part of the co-group implementation. It has
 * an embedded physical plan that generates tuples of the form (grpKey,(indxed
 * inp Tuple)).
 * 
 */
public class POLocalRearrange extends PhysicalOperator {
  ...
  // Example: a = load 'bla'; b = load 'bla'; c = cogroup a by ($2, $3), b by
  // ($, $2)
  // For the first input (a), the map would contain following key:value
  // 2:0 (2 corresponds to $2 in cogroup a by ($2, $3) and 0 corresponds to 1st
  // index in key)
  // 3:1 (3 corresponds to $3 in cogroup a by ($2, $3) and 0 corresponds to 2nd
  // index in key)
   ...
   * 
   * @param index
   *          the position of this operator in a co-group operation
   * @throws ExecException
   *           if the index value is bigger then 0x7F
   */
   ...
   * 
   * @param index
   *          the position of the parent plan of this operator in the enclosed
   *          split operator
   * @throws ExecException
   *           if the index value is bigger then 0x7F
   */
  ...
  private void setIndex(int index, boolean multiQuery) throws ExecException {
    if (index > PigNullableWritable.idxSpace) {
      ...
      // inputs)
      int errCode = 1082;
      String msg = multiQuery ? "Merge more than 127 map-reduce jobs not supported."
      ...
      this.index = multiQuery ? (byte) (index | PigNullableWritable.mqFlag)
          : (byte) index;
    }
  }
  ...
  /**
   * TODO Overridden since the attachment of the new input should cause the old
   * processing to end.
   */
  ...
  public Result getNextTuple() throws ExecException {
    ...
    while (true) {
      ...
      res.result = constructLROutput(resLst, secondaryResLst,
          (Tuple) inp.result);
  ...
  protected Tuple constructLROutput(List<Result> resLst,
      List<Result> secondaryResLst, Tuple value) throws ExecException {
  ...
  public POLocalRearrange clone() throws CloneNotSupportedException {
    List<PhysicalPlan> clonePlans = new ArrayList<PhysicalPlan>(plans.size());
    for (PhysicalPlan plan : plans) {
      clonePlans.add(plan.clone());
    }
    POLocalRearrange clone = new POLocalRearrange(new OperatorKey(mKey.scope,
  ...
  public void setPlansFromCombiner(List<PhysicalPlan> plans)
    ...
    for (PhysicalPlan plan : plans) {
      ...
      if (!isCross) {
        ...
        // the position in the "Key" where these will be projected to.
        // Then we can use this information to strip off these columns
        // from the "Value" and in POPostCombinerPackage stitch the right
        // "Value"
        // tuple back by getting these columns from the "key". The goal
        // is reduce the amount of the data sent to Hadoop in the map.
        if (leaf instanceof POProject) {
