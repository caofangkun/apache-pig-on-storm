...
import org.apache.storm.executionengine.physicalLayer.relationalOperators.POMergeJoin.TuplesToSchemaTupleList;
...
public class POFRJoin extends PhysicalOperator {
  ...
  public POFRJoin(OperatorKey k, int rp, List<PhysicalOperator> inp,
      ...
      FileSpec[] replFiles, int fragment, boolean isLeftOuter, Tuple nullTuple)
  ...
  public POFRJoin(OperatorKey k, int rp, List<PhysicalOperator> inp,
      List<List<PhysicalPlan>> ppLists, List<List<Byte>> keyTypes,
      FileSpec[] replFiles, int fragment, boolean isLeftOuter, Tuple nullTuple,
      Schema[] inputSchemas, Schema[] keySchemas) throws ExecException {
    super(k, rp, inp);
  ...
  private void setUpHashMap() throws ExecException {
    ...
    for (int i = 0; i < inputSchemas.length; i++) {
      ...
      if (schema != null) {
        ...
        inputSchemaTupleFactories[i] = SchemaTupleBackend
            .newSchemaTupleFactory(schema, false, GenContext.FR_JOIN);
      }
      schema = keySchemas[i];
      if (schema != null) {
        log.debug("Using SchemaTuple for FR Join key Schema: " + schema);
        keySchemaTupleFactories[i] = SchemaTupleBackend.newSchemaTupleFactory(
            schema, false, GenContext.FR_JOIN);
      }
    }
    ...
    for (FileSpec replFile : replFiles) {
      ...
      for (Result res = lr.getNextTuple(); res.returnStatus != POStatus.STATUS_EOP; res = lr
          .getNextTuple()) {
        if (getReporter() != null)
