...
public class POCast extends ExpressionOperator {
  ...
  // TODO: add by jianjzhang this is a short path for resoving problem:bytearray
  // from udf can not convert to other type.
  private void setDefaultCaster() {
  ...
  public String name() {
    if (DataType.isSchemaType(resultType))
      return "Cast" + "[" + DataType.findTypeName(resultType) + ":"
          + fieldSchema.calcCastString() + "]" + " - " + mKey.toString();
    else
      return "Cast" + "[" + DataType.findTypeName(resultType) + "]" + " - "
          + mKey.toString();
  }
    ...
    case DataType.BYTEARRAY: {
      ...
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        try {
          ...
          // res.result is not of type ByteArray. But it can be one of
          // the types from which cast is still possible.
          if (realType == null) {
  ...
  public Result getNextBoolean() throws ExecException {
    ...
    case DataType.DOUBLE: {
      ...
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        res.result = Boolean
            .valueOf(((Double) res.result).doubleValue() != 0.0);
      }
      return res;
    }
    ...
    case DataType.BIGINTEGER: {
      ...
      Result res = in.getNextBigInteger();
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        res.result = Boolean.valueOf(!BigInteger.ZERO
            .equals((BigInteger) res.result));
      }
      return res;
    }
    ...
    case DataType.BIGDECIMAL: {
      ...
      Result res = in.getNextBigDecimal();
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        res.result = Boolean.valueOf(!BigDecimal.ZERO
            .equals((BigDecimal) res.result));
      }
      return res;
    }
  ...
  public Result getNextInteger() throws ExecException {
    ...
    case DataType.DATETIME: {
      ...
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        res.result = Integer.valueOf(Long.valueOf(
            ((DateTime) res.result).getMillis()).intValue());
      }
      return res;
    }
  ...
  public Result getNextDouble() throws ExecException {
    ...
    case DataType.DATETIME: {
      Result res = in.getNextDateTime();
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        res.result = new Double(Long.valueOf(
            ((DateTime) res.result).getMillis()).doubleValue());
      }
      return res;
    }
  ...
  public Result getNextFloat() throws ExecException {
    ...
    case DataType.DATETIME: {
      ...
      Result res = in.getNextDateTime();
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        res.result = new Float(Long
            .valueOf(((DateTime) res.result).getMillis()).floatValue());
      }
      return res;
    }
    ...
    case DataType.TUPLE: {
      ...
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        try {
          ...
          LogUtils.warn(
              this,
              "Unable to interpret value " + res.result + " in field being "
      ...
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        ...
        try {
          ...
          if (DataType.findType(res.result) == castToType) {
            ...
            int errCode = 1081;
            String msg = "Cannot cast to tuple. Expected bytearray but received: "
                + DataType.findTypeName(res.result);
  ...
  private Object convertWithSchema(Object obj, ResourceFieldSchema fs)
      throws IOException {
    Object result = null;
    ...
    case DataType.BAG:
      if (obj instanceof DataBag) {
        ...
        throw new ExecException("Cannot cast " + obj + " to bag.", 1120,
            PigException.INPUT);
      }
      break;
    case DataType.TUPLE:
      if (obj instanceof Tuple) {
        ...
        throw new ExecException("Cannot cast " + obj + " to tuple.", 1120,
            PigException.INPUT);
      }
      break;
    case DataType.MAP:
      if (obj instanceof Map) {
        if (fs != null && fs.getSchema() != null) {
          ...
          for (Object entry : m.entrySet()) {
            Object newValue = convertWithSchema(((Map.Entry) entry).getValue(),
                innerFieldSchema);
        ...
        if (null != caster) {
          try {
            ...
            // this is for backward compatibility wherein some old
            // LoadCaster
            // which does not implement bytesToMap(byte[] b,
            // ResourceFieldSchema fieldSchema)
            // In this case, we only cast bytes to map, but leave
            // the value as bytearray
        ...
        throw new ExecException("Cannot cast " + obj + " to map.", 1120,
            PigException.INPUT);
      }
      break;
    case DataType.BOOLEAN:
      ...
      case DataType.BIGDECIMAL:
        ...
        break;
      default:
        throw new ExecException("Cannot convert " + obj + " to " + fs, 1120,
            PigException.INPUT);
      }
      break;
    case DataType.INTEGER:
      ...
      case DataType.FLOAT:
        ...
        break;
      case DataType.DATETIME:
        result = Integer.valueOf(Long.valueOf(((DateTime) obj).getMillis())
            .intValue());
        break;
      case DataType.CHARARRAY:
    ...
    case DataType.DOUBLE:
      ...
      case DataType.FLOAT:
        ...
        break;
      case DataType.DATETIME:
        result = new Double(Long.valueOf(((DateTime) obj).getMillis())
            .doubleValue());
        break;
      case DataType.CHARARRAY:
    ...
    case DataType.FLOAT:
      ...
      case DataType.FLOAT:
        ...
        break;
      case DataType.DATETIME:
        result = new Float(Long.valueOf(((DateTime) obj).getMillis())
            .floatValue());
        break;
      case DataType.CHARARRAY:
    ...
    default:
      throw new ExecException("Don't know how to convert " + obj + " to " + fs,
      ...
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        ...
        try {
          ...
          if (DataType.findType(res.result) == castToType) {
            ...
            // rethrow the exception
            int errCode = 1081;
            String msg = "Cannot cast to bag. Expected bytearray but received: "
                + DataType.findTypeName(res.result);
            throw new ExecException(msg, errCode, PigException.INPUT, e);
          }
      ...
      if (res.returnStatus == POStatus.STATUS_OK && res.result != null) {
        ...
        try {
          ...
          if (DataType.findType(res.result) == castToType) {
            ...
            // rethrow the exception
            int errCode = 1081;
            String msg = "Cannot cast to map. Expected bytearray but received: "
                + DataType.findTypeName(res.result);
            throw new ExecException(msg, errCode, PigException.INPUT, e);
          }
        ...
        try {
          if (null != caster) {
            try {
              ...
              // this is for backward compatibility wherein some
              // old LoadCaster
              // which does not implement bytesToMap(byte[] b,
              // ResourceFieldSchema fieldSchema)
              // In this case, we only cast bytes to map, but
              // leave the value as bytearray
