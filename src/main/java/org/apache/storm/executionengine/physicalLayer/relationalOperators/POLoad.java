...
/**
 * The load operator which is used in two ways: 1) As a local operator it can be
 * used to load files 2) In the Map Reduce setting, it is used to create jobs
 * from MapReduce operators which keep the loads and stores in the Map and
 * Reduce Plans till the job is created
 * 
 */
public class POLoad extends PhysicalOperator {
  ...
  /**
   * Set up the loader by 1) Instantiating the load func 2) Opening an input
  ...
  public void setUp() throws IOException {
    loader = new ReadToEndLoader(
        ...
        null, lFile.getFileName(), 0, signature);
  }
   ...
   * specified file using the specified load function.
   * 
   * @return Whatever the loader returns A null from the loader is indicative of
   *         EOP and hence the tearDown of connection
   */
  @Override
  ...
  public LoadFunc getLoadFunc() {
    if (this.loader == null) {
      this.loader = (LoadFunc) PigContext.instantiateFuncFromSpec(lFile
          .getFuncSpec());
