...
public class POCross extends PhysicalOperator {
  ...
  public Result getNextTuple() throws ExecException {
    ...
    if (its != null) {
      ...
      if (empty) {
        ...
        for (Result resOfLastBag = inputs.get(index).getNextTuple(); resOfLastBag.returnStatus != POStatus.STATUS_EOP; resOfLastBag = inputs
            .get(index).getNextTuple())
          ;
        res.returnStatus = POStatus.STATUS_EOP;
        clearMemory();
        return res;
  ...
  private boolean loadLastBag() throws ExecException {
    ...
    int index = inputs.size() - 1;
    for (resOfLastBag = inputs.get(index).getNextTuple(); resOfLastBag.returnStatus == POStatus.STATUS_NULL; inputs
        .get(index).getNextTuple())
      ;
