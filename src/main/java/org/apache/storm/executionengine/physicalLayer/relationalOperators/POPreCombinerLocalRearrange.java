...
/**
 * A specialized local rearrange operator which behaves like the regular local
 * rearrange in the getNext() as far as getting its input and constructing the
 * "key" out of the input. It then returns a tuple with two fields - the key in
 * the first position and the "value" inside a bag in the second position. This
 * output format resembles the format out of a Package. This output will feed to
 * a foreach which expects this format.
 */
public class POPreCombinerLocalRearrange extends PhysicalOperator {
  private static final Log log = LogFactory
      .getLog(POPreCombinerLocalRearrange.class);
