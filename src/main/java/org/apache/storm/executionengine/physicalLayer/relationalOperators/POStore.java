...
/**
 * The store operator which is used in two ways: 1) As a local operator it can
 * be used to store files 2) In the Map Reduce setting, it is used to create
 * jobs from MapReduce operators which keep the loads and stores in the Map and
...
public class POStore extends PhysicalOperator {
  ...
  public String name() {
    return (sFile != null) ? getAliasString() + "Store" + "("
        + sFile.toString() + ")" + " - " + mKey.toString() : getAliasString()
        + "Store" + "(" + "DummyFil:DummyLdr" + ")" + " - " + mKey.toString();
  }
  ...
  public StoreFuncInterface getStoreFunc() {
    if (storer == null) {
      storer = (StoreFuncInterface) PigContext.instantiateFuncFromSpec(sFile
          .getFuncSpec());
