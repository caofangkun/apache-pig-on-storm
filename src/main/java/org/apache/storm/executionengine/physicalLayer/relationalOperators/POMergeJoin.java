 ...
 * Currently, only two-way joins are supported. One input of join is identified
 * as left and other is identified as right. Left input tuples are the input
 * records in map. Right tuples are read from HDFS by opening right stream.
 * 
 * This join doesn't support outer join. Data is assumed to be sorted in
 * ascending order. It will fail if data is sorted in descending order.
 */
...
public class POMergeJoin extends PhysicalOperator {
   ...
   * should decrease the amount of memory needed for a given map task to
   * successfully perform a merge join.
   */
   ...
   * @param inp
   * @param inpPlans
   *          there can only be 2 inputs each being a List<PhysicalPlan> Ex.
   *          join A by ($0,$1), B by ($1,$2);
   */
  public POMergeJoin(OperatorKey k, int rp, List<PhysicalOperator> inp,
      MultiMap<PhysicalOperator, PhysicalPlan> inpPlans,
      List<List<Byte>> keyTypes, LOJoin.JOINTYPE joinType,
      Schema leftInputSchema, Schema rightInputSchema, Schema mergedInputSchema)
  ...
  private void createJoinPlans(
    ...
    for (PhysicalOperator inpPhyOp : inpPlans.keySet()) {
      ...
      lr.setKeyType(keyTypes.get(i).size() > 1 ? DataType.TUPLE : keyTypes.get(
          i).get(0));
  ...
  private void prepareTupleFactories() {
    ...
    if (leftInputSchema != null) {
      leftTupleMaker = SchemaTupleBackend.newSchemaTupleFactory(
          leftInputSchema, false, GenContext.MERGE_JOIN);
    }
    if (leftTupleMaker == null) {
      log.debug("No SchemaTupleFactory available for combined left merge join schema: "
          + leftInputSchema);
      leftTupleMaker = mTupleFactory;
    } else {
      log.debug("Using SchemaTupleFactory for left merge join schema: "
          + leftInputSchema);
    }

    if (mergedInputSchema != null) {
      mergedTupleMaker = SchemaTupleBackend.newSchemaTupleFactory(
          mergedInputSchema, false, GenContext.MERGE_JOIN);
    }
    if (mergedTupleMaker == null) {
      log.debug("No SchemaTupleFactory available for combined left/right merge join schema: "
          + mergedInputSchema);
      mergedTupleMaker = mTupleFactory;
    } else {
      log.debug("Using SchemaTupleFactory for left/right merge join schema: "
          + mergedInputSchema);
    }
  ...
  /**
   * This provides a List to store Tuples in. The implementation of that list
   * depends on whether or not there is a TupleFactory available.
   * 
  ...
  /**
   * This is a class that extends ArrayList, making it easy to provide on the
   * fly conversion from Tuple to SchemaTuple. This is necessary because we are
   * not getting SchemaTuples from the source, though in the future that is what
   * we would like to do.
   */
  ...
  public Result getNextTuple() throws ExecException {
    ...
    if (firstTime) {
      ...
      if (curLeftInp.returnStatus != POStatus.STATUS_OK)
        return curLeftInp; // Return because we want to fetch next left
      // tuple.
    ...
    if (doingJoin) {
      ...
      if (counter > 0) { // We have left tuples to join with current right
        // tuple.
      ...
      while (true) {
        ...
        if (rightInp.returnStatus != POStatus.STATUS_OK) {
          ...
          if (null == rightKey) // If we see tuple having null keys in
            // stream, we drop them
          ...
          if (cmpval == 0) {
            ...
            else { // This is end of all input and this is last join
              // output.
              // Right loader in this case wouldn't get a chance
              // to close input stream. So, we close it ourself.
              try {
                ((IndexableLoadFunc) rightLoader).close();
              } catch (IOException e) {
                // Non-fatal error. We can continue.
                log.error("Received exception while trying to close right side file: "
                    + e.getMessage());
              }
            }
            return new Result(POStatus.STATUS_EOP, null);
    ...
    case POStatus.STATUS_EOP:
      if (this.parentPlan.endOfAllInput) {
        ...
        // Fetch next left input.
        return curLeftInp;

    default: // If encountered with ERR / NULL on left side, we send it
      // down.
      return curLeftInp;
    }
    ...
    if ((null != prevRightKey) && !this.parentPlan.endOfAllInput
      ...
      // In that case, throw away the tuples accumulated till now and add
      // the one we read in this function call.
      leftTuples = newLeftTupleArray();
      leftTuples.add((Tuple) curLeftInp.result);
    ...
    // But since we are reading ahead we still haven't checked the read
    // ahead right tuple.
    // Accumulated left tuples may potentially join with that. So, lets
    // check that first.
    ...
    while (true) {
      // Start moving on right stream to find the tuple whose key is same
      // as with current left bag key.
      Result rightInp;
      ...
      if (cmpval < 0) { // still behind the left side, do nothing, fetch
        // next right tuple.
        ...
        // the buffered left tuples and add the one read in this
        // function call.
        leftTuples = newLeftTupleArray();
        leftTuples.add((Tuple) curLeftInp.result);
        prevLeftInp = curLeftInp;
        prevLeftKey = curLeftKey;
        if (this.parentPlan.endOfAllInput) { // This is end of all input
          // and this is last time
          // we will read right
          // input.
          // Right loader in this case wouldn't get a chance to close
          // input stream. So, we close it ourself.
          try {
            ((IndexableLoadFunc) rightLoader).close();
          } catch (IOException e) {
            // Non-fatal error. We can continue.
  ...
  private void seekInRightStream(Object firstLeftKey) throws IOException {
    rightLoader = (LoadFunc) PigContext
        .instantiateFuncFromSpec(rightLoaderFuncSpec);
  ...
  private Result getNextRightInp(Object leftKey) throws ExecException {
    ...
    /*
     * Only call seekNear if the merge join is 'merge-sparse'.
     * DefaultIndexableLoader does not support more than a single call to
     * seekNear per split - so don't call seekNear.
     */
    if (joinType == LOJoin.JOINTYPE.MERGESPARSE) {
      try {
        ((IndexableLoadFunc) rightLoader)
            .seekNear(leftKey instanceof Tuple ? (Tuple) leftKey
                : mTupleFactory.newTuple(leftKey));
        prevRightKey = null;
      } catch (IOException e) {
        throwProcessingException(true, e);
      }
  ...
  public void throwProcessingException(boolean withCauseException, Exception e)
      throws ExecException {
    int errCode = 2176;
  ...
  private Object extractKeysFromTuple(Result inp, int lrIdx)
      throws ExecException {
  ...
  public void setupRightPipeline(PhysicalPlan rightPipeline)
    ...
    if (rightPipeline != null) {
      if (rightPipeline.getLeaves().size() != 1
          || rightPipeline.getRoots().size() != 1) {
        int errCode = 2168;
  ...
  private OperatorKey genKey() {
    return new OperatorKey(opKey.scope, NodeIdGenerator.getGenerator()
        .getNextNodeId(opKey.scope));
  }
  ...
  public String name() {
    ...
    if (joinType == LOJoin.JOINTYPE.MERGESPARSE)
      name += "(sparse)";
    name += "[" + DataType.findTypeName(resultType) + "]" + " - "
        + mKey.toString();
    return name;
  }
