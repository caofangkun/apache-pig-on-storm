package org.apache.storm.executionengine.physicalLayer.util;
...
public class PlanHelper {
  ...
  /**
   * Creates a relative path that can be used to build a temporary place to
   * store the output from a number of map-reduce tasks.
   */
  ...
  public static <C extends PhysicalOperator> boolean containsPhysicalOperator(
      PhysicalPlan plan, Class<C> opClass) throws VisitorException {
  ...
  /**
   * Returns a LinkedList of operators contained within the physical plan which
   * implement the supplied class, in dependency order. Returns an empty
   * LinkedList of no such operators exist.
   * 
   * @param plan
   * @param opClass
   * @return a LinkedList of operators contained within the plan which implement
   *         the supplied class; empty if no such ops exist.
   * @throws VisitorException
   */
  public static <C extends PhysicalOperator> LinkedList<C> getPhysicalOperators(
      PhysicalPlan plan, Class<C> opClass) throws VisitorException {
    OpFinder<C> finder = new OpFinder<C>(plan, opClass);
    finder.visit();
  ...
  private static class OpFinder<C extends PhysicalOperator> extends
      PhyPlanVisitor {
    ...
    public OpFinder(PhysicalPlan plan, Class<C> opClass) {
      super(plan, new DependencyOrderWalker<PhysicalOperator, PhysicalPlan>(
          plan));
