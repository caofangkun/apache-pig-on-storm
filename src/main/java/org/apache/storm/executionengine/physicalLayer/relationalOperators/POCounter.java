...
/**
 * This operator is part of the RANK operator implementation. It adds a local
 * counter and a unique task id to each tuple. There are 2 modes of operations:
 * regular and dense. The local counter is depends on the mode of operation.
 * With regular rank is considered duplicate rows while assigning numbers to
 * distinct values groups. With dense rank counts the number of distinct values,
 * without considering duplicate rows. Depending on if it is considered. the
 * entire tuple (row number) or a by a set of columns (rank by).
 * 
 * This Physical Operator relies on some specific MR class, available at
 * PigMapReduceCounter.
 **/
...
public class POCounter extends PhysicalOperator {
  ...
  /**
   * In case of RANK BY, it could by dense or not. Being a dense rank means to
   * assign consecutive ranks to different values.
   **/
  private boolean isDenseRank = false;

  /**
   * In case of simple RANK, namely row number mode which is a consecutive
   * number assigned to each tuple.
   **/
  ...
  /**
   * Unique identifier that links POCounter and PORank, through the global
   * counter labeled with it.
   **/
  ...
  public POCounter(OperatorKey operatorKey, int requestedParallelism, List inp,
      List<PhysicalPlan> counterPlans, List<Boolean> ascendingCol) {
   ...
   * @return a tuple within two values prepended to the tuple the task
   *         identifier and the local counter value. Local counter value could
   *         be incremented by one (is a row number or dense rank) or, could be
   *         incremented by the size of the bag on the previous tuple processed
   **/
  protected Result addCounterValue(Result input) throws ExecException {
    ...
    // On this case, each tuple is analyzed independently of the tuples
    // grouped
    if (isRowNumber() || isDenseRank()) {
      ...
      if (in.getType(positionBag) == DataType.BAG) {
        sizeBag = ((org.apache.pig.data.DefaultAbstractBag) in.get(positionBag))
            .size();
      }
