...
public class POJoinPackage extends POPackage {
  ...
  public POJoinPackage(OperatorKey k, int rp, POPackage p, POForEach f) {
    ...
    forEach = new POOptimizedForEach(new OperatorKey(scope,
  ...
  /**
   * Calls getNext to get next ForEach result. The input for POJoinPackage is a
   * (key, NullableTuple) pair. We will materialize n-1 inputs into bags, feed
   * input#n one tuple a time to the delegated ForEach operator, the input for
   * ForEach is
   * 
   * (input#1, input#2, input#3....input#n[i]), i=(1..k), suppose input#n
   * consists
   * 
   * of k tuples. For every ForEach input, pull all the results from ForEach.
   * getNext will be called multiple times for a particular input, it returns
   * one output tuple from ForEach every time we call getNext, so we need to
   * maintain internal status to keep tracking of where we are.
   */
  @Override
  public Result getNextTuple() throws ExecException {
    ...
    // After this block, we have the first tuple of input#n in hand (kept in
    // variable it)
    if (newKey) {
