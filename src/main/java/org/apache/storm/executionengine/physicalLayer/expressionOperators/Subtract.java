...
public class Subtract extends BinaryExpressionOperator {
  ...
  @Override
  public String name() {
    return "Subtract" + "[" + DataType.findTypeName(resultType) + "]" + " - "
        + mKey.toString();
  }

  /*
   * This method is used to invoke the appropriate subtraction method, as Java
   * does not provide generic dispatch for it.
   */
  protected Number subtract(Number a, Number b, byte dataType)
  ...
  public Subtract clone() throws CloneNotSupportedException {
    Subtract clone = new Subtract(new OperatorKey(mKey.scope, NodeIdGenerator
