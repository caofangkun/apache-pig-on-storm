...
public class Divide extends BinaryExpressionOperator {
  ...
  @Override
  public String name() {
    return "Divide" + "[" + DataType.findTypeName(resultType) + "]" + " - "
        + mKey.toString();
  }

  /*
   * This method is used to invoke the appropriate method, as Java does not
   * provide generic dispatch for it.
   */
  protected Number divide(Number a, Number b, byte dataType)
  ...
  protected Result genericGetNext(byte dataType) throws ExecException {
    ...
    if (equalsZero(right, dataType)) {
      if (pigLogger != null) {
        pigLogger.warn(this, "Divide by zero. Converting it to NULL.",
            PigWarning.DIVIDE_BY_ZERO);
      }
      res.result = null;
    } else {
  ...
  public Divide clone() throws CloneNotSupportedException {
    Divide clone = new Divide(new OperatorKey(mKey.scope, NodeIdGenerator
