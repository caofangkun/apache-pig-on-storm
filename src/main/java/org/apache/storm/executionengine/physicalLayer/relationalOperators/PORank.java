...
/**
 * This operator is part of the RANK operator implementation. Reads the output
 * tuple from POCounter and the cumulative sum previously calculated. Here is
 * read the task identifier in order to get the corresponding cumulative sum,
 * and the local counter at the tuple. These values are summed and prepended to
 * the tuple.
 **/
...
public class PORank extends PhysicalOperator {
  ...
  /**
   * Reads the output tuple from POCounter and the cumulative sum previously
   * calculated. Here is read the task identifier in order to get the
   * corresponding cumulative sum, and the local counter at the tuple. These
   * values are summed and prepended to the tuple.
   * 
   * @param input
  ...
  public Result addRank(Result input) throws ExecException {
    ...
    // TODO
    String nameCounter = String.valueOf(taskId);
    Long rank = 0L;
    //
