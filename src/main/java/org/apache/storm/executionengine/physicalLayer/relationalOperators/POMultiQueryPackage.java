 ...
 * format as required by multi-query de-multiplexer.
 * <p>
 * This operator is used when merging multiple Map-Reduce splittees into a
 * Map-only splitter during multi-query optimization. The package operators of
 * the reduce plans of the splittees form an indexed package list inside this
 * operator. When this operator receives an input, it extracts the index from
 * the key and calls the corresponding package to get the output data.
 * <p>
 * Due to the recursive nature of multi-query optimization, this operator may be
 * contained in another multi-query packager.
 * <p>
 * The successor of this operator must be a PODemux operator which knows how to
 * consume the output of this operator.
 */
public class POMultiQueryPackage extends POPackage {
  ...
  /**
   * If the POLocalRearranges corresponding to the reduce plans in myPlans (the
   * list of inner plans of the demux) have different key types then the
   * MultiQueryOptimizer converts all the keys to be of type tuple by wrapping
   * any non-tuple keys into Tuples (keys which are already tuples are left
   * alone). The list below is a list of booleans indicating whether extra tuple
   * wrapping was done for the key in the corresponding POLocalRearranges and if
   * we need to "unwrap" the tuple to get to the key
   */
  private ArrayList<Boolean> isKeyWrapped = new ArrayList<Boolean>();

  /*
   * Indicating if all the inner plans have the same map key type. If not, the
   * keys passed in are wrapped inside tuples and need to be extracted out
   * during the reduce phase
   */
  private boolean sameMapKeyType = true;

  /*
   * Indicating if this operator is in a combiner. If not, this operator is in a
   * reducer and the key values must first be extracted from the tuple-wrap
  ...
  /**
   * Constructs an operator with the specified key, degree of parallelism and
   * inputs.
   * 
   * @param k
   *          the operator key
   ...
   * 
   * @param pack
   *          package to be appended to the list
   */
   ...
   *          package to be appended to the list
   * @param mapKeyType
   *          the map key type associated with the package
   */
   ...
   * <p>
   * The output is consumed by for the demultiplexer operator (PODemux) in the
   * format (key, {bag of tuples}) where key is an indexed WritableComparable,
   * not the wrapped value as a pig type.
   */
  @Override
  public Result getNextTuple() throws ExecException {
    ...
    if (obj instanceof PigNullableWritable) {
      ...
      if (obj == null) {
        ...
        myObj = HDataType.getWritableComparableTypes(obj,
            HDataType.findTypeFromNullableWritable(curKey));
      }
  ...
  /**
   * Returns the list of booleans that indicates if the key needs to unwrapped
   * for the corresponding plan.
   * 
   ...
   * 
   * @param lst
   *          the list of boolean values to add
   */
