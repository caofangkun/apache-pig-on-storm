...
public class POUserFunc extends ExpressionOperator {
  ...
  public Result processInput() throws ExecException {
    ...
    if (!initialized) {
      ...
      if (jobConf != null) {
        doTiming = "true"
            .equalsIgnoreCase(jobConf.get(TIME_UDFS_PROP, "false"));
  ...
  private Result getNext() throws ExecException {
    ...
    if (timeThis) {
      ...
      PigStatusReporter.getInstance()
          .getCounter(counterGroup, INVOCATION_COUNTER).increment(TIMING_FREQ);
      ...
      if (result.returnStatus == POStatus.STATUS_OK) {
        if (isAccumulative()) {
          if (isAccumStarted()) {
            ...
            if (!hasBeenTerminated() && isEarlyTerminating()
                && ((TerminatingAccumulator<?>) func).isFinished()) {
              earlyTerminate();
            }
