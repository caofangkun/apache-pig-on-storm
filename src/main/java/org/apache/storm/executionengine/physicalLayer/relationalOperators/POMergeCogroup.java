...
public class POMergeCogroup extends PhysicalOperator {
    ...
    try {
      ...
      case POStatus.STATUS_EOP:
        ...
        while (!heap.isEmpty()) {
          ...
          if (null == firstKeyOfNextSplit || null == key
              || firstKeyOfNextSplit.compareTo(key) > 0) {
        ...
        if (heap.isEmpty()
            || (firstKeyOfNextSplit != null && firstKeyOfNextSplit
                .compareTo(heap.peek().get(1)) <= 0))
          lastTime = true;
      ...
      // Every time we read something valid from mapper, we add it to
      // heap.
      ...
      /*
       * while (there are records in heap) { peek at top record from heap if we
       * see a key change from previous run, break to return output. else, pull
       * tuple from top of heap and place it in bag based on which input it came
       * from pull record from input that last record from top of heap came from
       * if (key pulled < first key in next split) insert into heap } output
       * final record
       */
      ...
      while (!heap.isEmpty()) {
        ...
        if (relIdx == 0) {
          ...
          /*
           * At this point, its possible to return the output tuple. So, lets
           * check if we can do so. Remember, that for tuples having equal keys,
           * tuple from leftmost relation is considered largest. So, if the
           * tuple we just peeked from heap is of leftmost relation and has a
           * different key then the tuple we polled last, then we can be assured
           * that we have seen all the tuples belonging to this key and thus we
           * can generate output tuple. Else, just pull the next tuple from the
           * left relation by returning EOP.
           */
          // First check if there is a tuple at top of heap and is
          // from
          // left relation.
          if ((null != tuple) && (Byte) tuple.get(0) == 0) {
            ...
            if ((prevTopOfHeap.get(1) == null && tuple.get(1) != null)
                || !tuple.get(1).equals(prevTopOfHeap.get(1))) {
              return getOutputTuple();
            }
          }
        ...
        if (firstKeyOfNextSplit == null || null == key
            || firstKeyOfNextSplit.compareTo(key) > 0) {
          heap.offer(rearrangedTup);
        }
      }
  ...
  private boolean needToBreak(final Tuple curTopOfHeap,
      final Tuple prevTopOfHeap) throws ExecException {
    ...
    if (curKey == null && null == prevKey)
      // If both keys are nulls, check if they are coming from same
      // relation.
    ...
    if (curKey == null || null == prevKey)
      // If only one of them is null, then key has changed from null to
      // non-null.
      return true;
  ...
  private void setup(Tuple firstRearrangedTup) throws IOException {
    ...
    for (int i = 0; i < relationCnt - 1; i++) {
      ...
      LoadFunc loadfunc = (LoadFunc) PigContext
          .instantiateFuncFromSpec(sidFuncSpecs.get(i));
      ...
      if (index.get(0).first.equals(curSplitIdx)) {
        // This is a first split, bind at very first record in all side
        // relations.
        ...
        ((IndexableLoadFunc) loadfunc)
            .seekNear(firstBaseKey instanceof Tuple ? (Tuple) firstBaseKey
                : mTupleFactory.newTuple(firstBaseKey));

        // Since contract of IndexableLoadFunc is not clear where we
        // will land up after seekNear() call,
        // we start reading from side loader to get to the point where
        // key
        ...
        while (true) {
          ...
          int cmpVal = ((Comparable<Object>) rearranged.get(1))
              .compareTo(firstBaseKey);
          if (cmpVal >= 0) { // Break away as soon as we get ahead.

            // Add this tuple in heap only if it needs to be
            // processed in
            // this map. That is it needs to be smaller then next
            // split's
            // first key, unless this is the last split, in which
            // case it
            // will be processed in this map.
            if (firstKeyOfNextSplit == null
                || firstKeyOfNextSplit.compareTo(rearranged.get(1)) > 0) {
              heap.offer(rearranged);
            }
            break;
          }
  ...
  private List<Pair<Integer, Tuple>> readIndex() throws ExecException {
    ...
    // Since nulls are smaller then anything else, if its in data, key with
    // null value
    // should be very first entry of index.

    // First create a loader to read index.
    POLoad ld = new POLoad(new OperatorKey(this.mKey.scope, NodeIdGenerator
        .getGenerator().getNextNodeId(this.mKey.scope)), new FileSpec(
        indexFileName, idxFuncSpec));
    ...
    for (Result res = ld.getNextTuple(); res.returnStatus != POStatus.STATUS_EOP; res = ld
      ...
      index.add(new Pair<Integer, Tuple>((Integer) idxTuple.get(colCnt + 1),
          keyTuple));
    }
  ...
  private Comparable<Object> getFirstKeyOfNextSplit(final int curSplitIdx,
    ...
    if (i < index.size() - 1) {
      ...
      return keyTuple.size() == 1 ? (Comparable<Object>) keyTuple.get(0)
          : keyTuple;
    }
  ...
  private Tuple applyLRon(final Tuple inp, final int lrIdx)
    ...
    this.heap = new PriorityQueue<Tuple>(11, new Comparator<Tuple>() {
      ...
      public int compare(final Tuple resLHS, final Tuple resRHS) {
        try {
          ...
          // Java's priority queue doesn't handle null, so we handle
          // it ourselves.

          // If both keys are null, then keys of side loader is
          // considered smaller.
          // That is so, because when we poll the heap, we want to get
          // the keys
          ...
          return cmpval == 0 ? ((Byte) resRHS.get(0)).compareTo((Byte) resLHS
              .get(0)) : cmpval;
