...
/**
 * Do partial aggregation in map plan. Inputs are buffered up in a hashmap until
 * a threshold is reached; then the combiner functions are fed these buffered up
 * inputs, and results stored in a secondary map. Once that map fills up or all
 * input has been seen, results are piped out into the next operator (caller of
 * getNext()).
 */
public class POPartialAgg extends PhysicalOperator implements Spillable {
  ...
  // Array Lists grow by prevSize + prevSize/2. Given default initial size of
  // 10,
  // 9369 is the size of the array after 18 such resizings. This seems like a
  // sufficiently
  ...
  private static final int SECOND_TIER_THRESHOLD = FIRST_TIER_THRESHOLD
  ...
  private void init() throws ExecException {
    ...
    initialized = true;
    // TODO:SpillableMemoryManager.getInstance().registerSpillable(this);
  }
  ...
  private void estimateMemThresholds() {
    if (!mapAggDisabled()) {
      ...
      LOG.info("Estimated total tuples to buffer, based on " + estTuples
          + " tuples that took up " + estTotalMem + " bytes: " + totalTuples);
      firstTierThreshold = (int) (0.5 + totalTuples
          * (1f - (1f / sizeReduction)));
      secondTierThreshold = (int) (0.5 + totalTuples * (1f / sizeReduction));
      LOG.info("Setting thresholds. Primary: " + firstTierThreshold
          + ". Secondary: " + secondTierThreshold);
    }
  ...
  private void checkSizeReduction() throws ExecException {
    ...
    LOG.info("After reduction, processed map: " + numRecsInProcessedMap
        + "; raw map: " + numRecsInRawMap);
    int minReduction = getMinOutputReductionFromProp();
    LOG.info("Observed reduction factor: from " + numBeforeReduction + " to "
        + numAfterReduction + " => " + numBeforeReduction / numAfterReduction
        + ".");
    if (numBeforeReduction / numAfterReduction < minReduction) {
      LOG.info("Disabling in-memory aggregation, since observed reduction is less than "
          + minReduction);
      disableMapAgg();
    }
  ...
  private void addKeyValToMap(Map<Object, List<Tuple>> map, Object key,
      Tuple inpTuple) throws ExecException {
    ...
    if (value.size() >= MAX_LIST_SIZE) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("The cache for key " + key
            + " has grown too large. Aggregating "
            + ((isFirst) ? "first level." : "second level."));
      }
      if (isFirst) {
        aggregateRawRow(key);
      } else {
  ...
  private void startSpill() throws ExecException {
    ...
    // to set up.
    if (spillingIterator != null)
      return;

    if (!rawInputMap.isEmpty()) {
      if (LOG.isInfoEnabled()) {
        LOG.info("In startSpill(), aggregating raw inputs. " + numRecsInRawMap
            + " tuples.");
      }
      aggregateFirstLevel();
      if (LOG.isInfoEnabled()) {
    ...
    if (!processedInputMap.isEmpty()) {
      if (LOG.isInfoEnabled()) {
        LOG.info("In startSpill(), aggregating processed inputs. "
  ...
  /**
   * For each entry in rawInputMap, feed the list of tuples into the aggregator
   * funcs and add the results to processedInputMap. Remove the entries from
   * rawInputMap as we go.
   * 
   * @throws ExecException
   */
  private int aggregate(Map<Object, List<Tuple>> fromMap,
      Map<Object, List<Tuple>> toMap, int numEntriesInTarget)
      throws ExecException {
    Iterator<Map.Entry<Object, List<Tuple>>> iter = fromMap.entrySet()
        .iterator();
    while (iter.hasNext()) {
  ...
  private void aggregateFirstLevel() throws ExecException {
    numRecsInProcessedMap = aggregate(rawInputMap, processedInputMap,
        numRecsInProcessedMap);
    numRecsInRawMap = 0;
  }

  private void aggregateSecondLevel() throws ExecException {
    Map<Object, List<Tuple>> newMap = Maps
        .newHashMapWithExpectedSize(processedInputMap.size());
    numRecsInProcessedMap = aggregate(processedInputMap, newMap, 0);
    processedInputMap = newMap;
  }

  private Tuple createValueTuple(Object key, List<Tuple> inpTuples)
      throws ExecException {
  ...
  private int getMinOutputReductionFromProp() {
    int minReduction = 0;// TODO
    if (minReduction <= 0) {
      LOG.info("Specified reduction is < 0 (" + minReduction
          + "). Using default " + DEFAULT_MIN_REDUCTION);
   ...
   * @param key
   * @param value
   * @return Result, containing a tuple of form (key, tupleReturnedByPlan1,
   *         tupleReturnedByPlan2, ...)
   * @throws ExecException
   */
